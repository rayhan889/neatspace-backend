...
func TestJWTGenerator(t *testing.T) {
	...
	t.Run("Sign_ParseValidate_ParseUnmarshal", func(t *testing.T) {
		...
		// Sign test
		tokenStr, err := jwtGen.Sign(ctx, payload, "test-user")
		require.NoError(t, err)
		require.NotEmpty(t, tokenStr)

		// ParseAndValidate test
		claims, err := jwtGen.ParseAndValidate(ctx, tokenStr)
		require.NoError(t, err)
		assert.Equal(t, "access", claims["typ"])
		assert.Equal(t, "jake", claims["name"])
		assert.Equal(t, "admin", claims["role"])
		assert.Equal(t, "test-issuer", claims[jwt.IssuerKey])
		assert.Equal(t, "test-user", claims[jwt.SubjectKey])

		expVal, ok := claims[jwt.ExpirationKey]
		...
		var out outClaims
		err = jwtGen.ParseAndUnmarshal(ctx, tokenStr, &out)
		require.NoError(t, err)
		assert.Equal(t, "jake", out.Name)
		assert.Equal(t, "admin", out.Role)
		assert.Equal(t, "test-user", out.Sub)
