...
const (
	migrationDir          = "files"
	databaseConnFailedMsg = "Database connection attempt failed"
)

// createConnection creates a new database connection pool and a SQL database instance with retry mechanism
func createConnection(databaseURL string) (*pgxpool.Pool, *sql.DB, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if err != nil {
			...
			slog.Warn(databaseConnFailedMsg, "attempt", attempt)
...
func NewMigrator(databaseURL string) *Migrator {
	pool, db, err := createConnection(databaseURL)
	if err != nil {
...
func (m *Migrator) MigrateUp(ctx context.Context) error {
	...
	if err := goose.UpContext(ctx, m.db, migrationDir); err != nil {
...
func (m *Migrator) MigrateReset(ctx context.Context) error {
	...
	if err := goose.Reset(m.db, migrationDir); err != nil {
...
func (m *Migrator) MigrateVersion(ctx context.Context) error {
	...
	if err := goose.Version(m.db, migrationDir); err != nil {
...
func (m *Migrator) MigrateCreate(ctx context.Context, migrationName string) error {
	if err := m.MigrateStatus(ctx); err != nil {
		slog.Error("Failed to show migration status before creating one", "err", err)
	}
	...
	migrationsDir := filepath.Join(execDir, migrationDir)
	if _, err := os.Stat(migrationsDir); os.IsNotExist(err) {
		// Try relative to this source file (for go run or test)
		migrationsDir = filepath.Join(execDir, "migrations", migrationDir)
		if _, err := os.Stat(migrationsDir); os.IsNotExist(err) {
			slog.Error("Migrations directory does not exist", "checked", []string{
				filepath.Join(execDir, migrationDir),
				filepath.Join(execDir, "migrations", migrationDir),
			})
			return fmt.Errorf("files directory does not exist")
		}
	}
